...
class Shipping_VendesFacil_WC extends WC_Shipping_Method_Shipping_VendeFacil_WC
{
    ...
    public function generate_transaction($order_id, $old_status, $new_status, $order)
    {
        ...
        if (empty($txid) && !empty($instance->license_key) && $new_status === 'processing'){
    ...
    public function upgrade_working_plugin()
    {
        if (!empty($this->license_key)){
            ...
            if (is_wp_error($response)){
                shipping_vendesfacil_wc_svwc_notices( $response->get_error_message() );
                return;
            }
