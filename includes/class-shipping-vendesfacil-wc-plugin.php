...
class Shipping_VendesFacil_WC_Plugin
{
    ...
    protected function _run()
    {
        ...
        $settings = get_option('woocommerce_shipping_vendesfacil_wc_settings' );

        if (empty($settings['license_key'])){
            $country = 'Shipping VendesFacil Woocommerce: Requiere una licencia para su completo funcionamiento '  .
                sprintf(
                    '%s',
                    '<a target="_blank" class="button button-primary"  href="https://shop.saulmoralespa.com/producto/plugin-shipping-vendesfacil-woocommerce/">' .
                    'Obtener Licencia</a>' );
            add_action(
                'admin_notices',
                function() use($country) {
                    shipping_vendesfacil_wc_svwc_notices( $country );
                }
            );
        }
    }
